...
function dataScore(value=data){const x=normalizeData(value);return Object.keys(x.tasks||{}).length*4+Object.keys(x.logs.words||{}).length*3+Object.keys(x.logs.listening||{}).length*3+Object.keys(x.logs.tests||{}).length*3+Math.max(0,(x.vocab||[]).length-defaultVocab.length)*2+(x.lesson?.number&&x.lesson.number!==1?3:0)+Object.values(x.lesson?.checks||{}).filter(Boolean).length}
...
function showBackup(){const backups=listLocalBackups();const backupHtml=backups.length?`<div class="restore-list"><p class="muted">发现 ${backups.length} 份同步前本机备份。若进度被云端覆盖，可先点“预览”，确认后恢复。</p>${backups.map((b,i)=>`<div class="restore-item"><div><strong>本机备份 ${i+1}</strong><div class="task-meta">${esc(b.time||'未知时间')} · 数据量 ${b.score}</div></div><div class="restore-actions"><button type="button" class="small-btn ghost" data-preview-backup="${esc(b.key)}">预览</button><button type="button" class="small-btn" data-restore-backup="${esc(b.key)}">恢复</button></div></div>`).join('')}</div>`:'';const c=openModal(`<h3>数据备份</h3><p>数据会优先保存在本机；登录后同步到云端。建议重要阶段继续导出备份。</p><div class="button-row"><button type="button" id="export" class="small-btn">导出数据</button><button type="button" id="import" class="small-btn ghost">导入数据</button></div>${backupHtml}<div class="modal-actions"><button class="small-btn ghost">关闭</button></div>`);c.querySelector('#export').onclick=exportData;c.querySelector('#import').onclick=()=>document.querySelector('#importInput').click();c.querySelectorAll('[data-preview-backup]').forEach(btn=>btn.onclick=()=>previewLocalBackup(btn.dataset.previewBackup));c.querySelectorAll('[data-restore-backup]').forEach(btn=>btn.onclick=()=>restoreLocalBackup(btn.dataset.restoreBackup))}
function previewLocalBackup(key){try{const value=normalizeData(JSON.parse(localStorage.getItem(key)||'{}'));alert(`备份预览\n教材：第 ${value.lesson.number||1} 课\n词库：${(value.vocab||[]).length} 词\n任务日期：${Object.keys(value.tasks||{}).length} 天\n听力记录：${Object.keys(value.logs?.listening||{}).length} 天\n测试记录：${Object.keys(value.logs?.tests||{}).length} 次`)}catch{alert('这份备份无法读取。')}}
function restoreLocalBackup(key){try{const value=normalizeData(JSON.parse(localStorage.getItem(key)||'{}'));if(!confirm('确定恢复这份本机备份吗？当前显示的数据会先自动再备份一份，然后用这份备份覆盖。'))return;makeLocalBackup('before-manual-restore');replaceData(value,{skipRender:true});save();document.querySelector('#modal').close();render();alert('已恢复本机备份，并会在登录状态下同步到云端。')}catch{alert('这份备份无法恢复。')}}
