...
function editVocab(id){
 ...
 c.querySelector('#saveV').onclick=e=>{e.preventDefault();const word=c.querySelector('#vw').value.trim(),kana=c.querySelector('#vk').value.trim(),meaning=c.querySelector('#vm').value.trim(),collocations=c.querySelector('#vc').value.trim();if(!word||!meaning){alert('请至少填写日语和中文意思。');return}if(existing){Object.assign(existing,{word,kana,meaning,collocations})}else{data.vocab.push({id:'v'+Date.now(),word,kana,meaning,collocations,mastery:0,wrong:0,created:todayKey()})}save();showVocab()};
}
function deleteVocab(id){const v=data.vocab.find(x=>x.id===id);if(!v||!confirm(`确定删除“${v.word}”吗？`))return;data.vocab=data.vocab.filter(x=>x.id!==id);save();showVocab()}
