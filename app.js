...
function blankData(){return{
  schemaVersion:4,updated_at:new Date().toISOString(),
  ...
  tasks:{},weekPlans:{},logs:{words:{},listening:{},tests:{},practice:{}},practiceTimer:null,vocab:defaultVocab,calendarMonth:null
}}
function normalizeData(raw={}){const base=blankData(), merged={...base,...raw};merged.settings={...base.settings,...(raw.settings||{})};merged.lesson={...base.lesson,...(raw.lesson||{})};merged.lesson.checks=normalizeLessonChecks((raw.lesson||{}).checks||{});merged.logs={words:{},listening:{},tests:{},...(raw.logs||{})};merged.logs.words=merged.logs.words||{};merged.logs.listening=merged.logs.listening||{};merged.logs.tests=merged.logs.tests||{};merged.logs.practice=merged.logs.practice||{};merged.practiceTimer=raw.practiceTimer&&raw.practiceTimer.startedAt?raw.practiceTimer:null;merged.tasks=merged.tasks||{};merged.weekPlans=merged.weekPlans||{};merged.vocab=Array.isArray(merged.vocab)?merged.vocab:base.vocab;merged.schemaVersion=4;merged.updated_at=raw.updated_at||raw.updatedAt||base.updated_at;return merged}
let data=load();
...
function createSync(){
 ...
 const state={available,client:null,user:null,status:available?'未登录':'本地',message:available?'登录后自动同步':'未配置 Supabase',timer:null,poller:null,busy:false,lastSync:null,error:null};
 if(available)state.client=window.supabase.createClient(cfg.url,cfg.anonKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true,storageKey:'jp-study-supabase-auth'}});
 ...
 function startAutoSync(){stopAutoSync();state.poller=setInterval(()=>pullThenPush(),20000)}
 function stopAutoSync(){if(state.poller){clearInterval(state.poller);state.poller=null}}
 async function init(){renderSyncStatus();window.addEventListener('online',()=>{if(state.user){setStatus('同步中','网络恢复，正在同步');pullThenPush()}});window.addEventListener('offline',()=>setStatus('离线','离线可用，联网后同步'));window.addEventListener('focus',()=>{if(state.user)pullThenPush()});document.addEventListener('visibilitychange',()=>{if(!document.hidden&&state.user)pullThenPush()});
  if(!state.available)return;
  try{const {data:sessionData}=await state.client.auth.getSession();state.user=sessionData.session?.user||null;if(state.user){startAutoSync();await firstSync()}else setStatus('未登录','登录后自动同步')}catch(err){setStatus('异常','同步初始化失败',err)}
  state.client.auth.onAuthStateChange((_event,session)=>{state.user=session?.user||null;if(state.user){startAutoSync();firstSync()}else{stopAutoSync();setStatus('未登录','已退出，仅保留本地数据')}});
 }
 ...
 async function signInPassword(email,password){if(!state.available)throw new Error('请先填写 Supabase 配置。');const {data:auth,error}=await state.client.auth.signInWithPassword({email,password});if(error)throw error;state.user=auth.session?.user||auth.user;startAutoSync();await firstSync()}
 async function signUpPassword(email,password){if(!state.available)throw new Error('请先填写 Supabase 配置。');const {data:auth,error}=await state.client.auth.signUp({email,password});if(error)throw error;if(auth.session?.user){state.user=auth.session.user;startAutoSync();await firstSync()}else setStatus('查收邮件','请先完成邮箱确认')}
 async function updatePassword(password){if(!state.available||!state.user)throw new Error('请先登录。');const {error}=await state.client.auth.updateUser({password});if(error)throw error;setStatus('已同步','密码已更新，手机可用密码登录')}
 ...
 async function firstSync(){if(state.busy)return;state.busy=true;try{if(!navigator.onLine){setStatus('离线','离线可用，联网后同步');return}setStatus('同步中','正在检查云端数据');const remote=await fetchRemote();const meta=loadSyncMeta();const migrated=meta.migratedUsers?.[state.user.id];
   ...
   if(!migrated&&hasMeaningfulLocalData()&&(localScore>remoteScore+2||(localIsNewer&&localScore>=remoteScore))){await upsertRemote(data);meta.migratedUsers={...(meta.migratedUsers||{}),[state.user.id]:true};saveSyncMeta(meta);return}
 ...
 async function pullThenPush(){if(!state.available||!state.user||state.busy)return;state.busy=true;try{if(!navigator.onLine){setStatus('离线','离线可用，联网后同步');return}const remote=await fetchRemote(),meta=loadSyncMeta(),pending=Boolean(meta.pending);if(!remote){await upsertRemote(data);return}const remoteData=normalizeData(remote.data),localScore=dataScore(data),remoteScore=dataScore(remoteData),remoteIsNewer=compareUpdated(remote.updated_at,data.updated_at)>0,localIsNewer=compareUpdated(data.updated_at,remote.updated_at)>0;if(remoteIsNewer||(!pending&&remoteScore>localScore+2)||(pending&&localIsNewer&&localScore+2<remoteScore)){makeLocalBackup('before-cloud-replace');replaceData(remoteData,{skipRender:true});meta.pending=false;saveSyncMeta(meta);setStatus('已同步','已载入另一台设备的新数据');render();return}if(pending||localIsNewer){await upsertRemote(data);return}meta.pending=false;saveSyncMeta(meta);setStatus('已同步','云端与本机一致')}catch(err){const meta=loadSyncMeta();meta.pending=true;saveSyncMeta(meta);setStatus('待同步','本机已保存，稍后自动同步',err)}finally{state.busy=false}}
 ...
 return {state,init,signIn,signInPassword,signUpPassword,updatePassword,signOut,queuePush,pullThenPush};
}
...
function render(){ensureToday();renderHeader();renderPracticeTimer();renderTasks();renderLesson();renderWords();renderListening();renderWeekend();renderCalendar();renderSummary();renderSyncStatus()}
...
function renderLesson(){document.querySelector('#lessonNumber').textContent=data.lesson.number;const box=document.querySelector('#lessonChecks');box.innerHTML='';lessonParts.forEach(p=>{const l=document.createElement('label');l.className='mini-check';l.innerHTML=`<input type="checkbox" ${data.lesson.checks[p]?'checked':''}> ${p}`;l.querySelector('input').onchange=e=>{data.lesson.checks[p]=e.target.checked;clearTimeout(lessonAdvanceTimer);save();renderLesson();renderSyncStatus();if(lessonParts.every(part=>data.lesson.checks[part])){const completed=data.lesson.number;lessonAdvanceTimer=setTimeout(()=>{if(!lessonParts.every(part=>data.lesson.checks[part]))return;data.lesson.number=completed+1;data.lesson.checks=Object.fromEntries(lessonParts.map(part=>[part,false]));save();renderLesson();renderSyncStatus();const note=document.querySelector('#lessonAdvanceNote');if(note)note.textContent=`第 ${completed} 課完了。第 ${completed+1} 課へ進みました。`},500)}};box.appendChild(l)});const n=lessonParts.filter(p=>data.lesson.checks[p]).length;document.querySelector('#lessonProgressBar').style.width=(n/lessonParts.length*100)+'%';let note=document.querySelector('#lessonAdvanceNote');if(!note){note=document.createElement('p');note.id='lessonAdvanceNote';note.className='lesson-advance-note';note.setAttribute('aria-live','polite');box.insertAdjacentElement('afterend',note)}}
...
document.querySelector('#syncBtn').onclick=()=>{const s=sync.state;const configured=s.available;const user=s.user;const err=s.error?`<p class="sync-error">${esc(s.error.message||s.error)}</p>`:'';const html=user?`<h3>ログインと同期</h3><p class="muted">${esc(user.email||'ログイン済み')}</p><p>${esc(s.message)}</p>${err}<div class="field"><label>スマホ用パスワード</label><input id="newPassword" type="password" autocomplete="new-password" minlength="6" placeholder="6文字以上"></div><div class="button-row"><button type="button" id="syncNow" class="small-btn">今すぐ同期</button><button type="button" id="setPassword" class="small-btn ghost">パスワード設定</button><button type="button" id="logout" class="small-btn ghost">ログアウト</button></div><div class="modal-actions"><button class="small-btn ghost">閉じる</button></div>`:`<h3>ログインと同期</h3><p>${configured?'同じメールとパスワードでログインすると、スマホとパソコンが自動同期されます。':'Supabase が未設定です。データはこの端末に保存されます。'}</p>${err}${configured?'<div class="field"><label>メール</label><input id="email" type="email" autocomplete="email" placeholder="you@example.com"></div><div class="field"><label>パスワード</label><input id="password" type="password" autocomplete="current-password" minlength="6" placeholder="6文字以上"></div><div class="button-row"><button type="button" id="passwordLogin" class="small-btn">ログイン</button><button type="button" id="passwordSignup" class="small-btn ghost">新規登録</button></div><div class="modal-actions"><button value="cancel" class="small-btn ghost">閉じる</button><button type="button" id="sendLogin" class="small-btn ghost">メールリンク</button></div>':'<div class="modal-actions"><button class="small-btn ghost">閉じる</button></div>'}`;const c=openModal(html);const emailValue=()=>c.querySelector('#email').value.trim();const passwordValue=()=>c.querySelector('#password').value;const login=c.querySelector('#passwordLogin');if(login)login.onclick=async()=>{const email=emailValue(),password=passwordValue();if(!email||!password)return alert('メールとパスワードを入力してください。');try{await sync.signInPassword(email,password);alert('ログインしました。同期を開始します。');document.querySelector('#modal').close();render()}catch(err){alert(err.message||'ログインに失敗しました')}};const signup=c.querySelector('#passwordSignup');if(signup)signup.onclick=async()=>{const email=emailValue(),password=passwordValue();if(!email||password.length<6)return alert('メールと6文字以上のパスワードを入力してください。');try{await sync.signUpPassword(email,password);alert('登録しました。必要なら確認メールを開いてください。');document.querySelector('#modal').close();render()}catch(err){alert(err.message||'登録に失敗しました')}};const setPassword=c.querySelector('#setPassword');if(setPassword)setPassword.onclick=async()=>{const password=c.querySelector('#newPassword').value;if(password.length<6)return alert('6文字以上のパスワードを入力してください。');try{await sync.updatePassword(password);alert('パスワードを設定しました。スマホで同じメールとこのパスワードでログインしてください。')}catch(err){alert(err.message||'設定に失敗しました')}};const send=c.querySelector('#sendLogin');if(send)send.onclick=async()=>{const email=emailValue();if(!email)return alert('メールを入力してください。');try{await sync.signIn(email);alert('ログインメールを送信しました。メール内のリンクを開いてください。')}catch(err){alert(err.message||'送信に失敗しました')}};const syncNow=c.querySelector('#syncNow');if(syncNow)syncNow.onclick=()=>sync.pullThenPush();const logout=c.querySelector('#logout');if(logout)logout.onclick=()=>sync.signOut()};
...
document.addEventListener('visibilitychange',()=>{if(!document.hidden)updatePracticeTimerDisplay()});
